...
public class ATRParameterTest { 
    ...
    public void testParameter(){ 
        ...
        Value value = DataService.instance().value(dt2.toInstant(), new ATRParameter(new Security("IBM", "TEST"), TimeInterval.DAY, 14)); 
        assertThat(value.isOK(), is(equalTo(TRUE))); 
        assertThat(value.getValue(), is(closeTo(1.318371911, 1e-6))); 
    } 
 
    @Test 
    public void testParameterNA(){ 
        DateTime dt2 = new DateTime(2010, 12, 26, 0, 0, 0, 0, DateTimeZone.UTC); 
        Value value = DataService.instance().value(dt2.toInstant(), new ATRParameter(new Security("IBM", "TEST"), TimeInterval.DAY, 14)); 
