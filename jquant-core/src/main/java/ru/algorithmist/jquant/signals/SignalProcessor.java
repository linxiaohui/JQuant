...
public class SignalProcessor { 
    ...
    public void process(ISignal signal, Instant from, Instant to, SignalCallback callback, OpenPositions openPositions, Cash cash){ 
        if (to.isBefore(from)){ 
            throw new IllegalArgumentException("From date should be before to date"); 
        } 
        List<SignalData> dataL = signal.test(from, to); 
        for(SignalData data : dataL) { 
